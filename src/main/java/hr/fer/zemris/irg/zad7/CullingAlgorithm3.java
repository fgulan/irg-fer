 ...
 * @author Filip Gulan
 */
public class CullingAlgorithm3 implements ICulling {
    ...
    public void renderScene(GL2 gl2, ObjectModel model, IMatrix matrix, IVector eye) {
        ...
        for (Face3D face3D : model.getFaces()) {
            List<IVector> tPoints = new ArrayList<>(3);
            ...
            boolean isAntiClockwise = IRG.isAntiClockwise(tPoints.get(0), tPoints.get(1), tPoints.get(2));
            if (!isAntiClockwise) {
                continue;
            }
            gl2.glBegin(GL2.GL_LINE_LOOP);
            for (IVector tPoint : tPoints) {
                gl2.glVertex2f((float) tPoint.get(0), (float) tPoint.get(1));
            }
            gl2.glEnd();
        }
    }
